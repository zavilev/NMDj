...
if __name__ == '__main__':
    ...
    #    ann = nmdj.load_annotation(args.ann)
        ...
        gtf = gtf[gtf.transcript_id.isin(nset)]
    ...
    #assign novel transcripts to genes
        ...
        print(f'Novel transcripts assigned to genes, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)
    ...
    #find ORFs
        ...
        print(f'ORFs found and written to {out}, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)

        if not args.nmd:
            warnings.warn("--nmd is not set, although novel transcripts are given. Gonna find NDMTs anyway)")
        nset = set(orf.transcript_id)-set(ann.transcript_id)
        gtf = gtf[gtf.transcript_id.isin(nset)]
    ...
    if args.nmd:
        ...
        print(f'NMDTs found and written to {out}, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)
    ...
    print(f'GTF filtered, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)



    #calculate frame
    gtf = nmdj.calculate_frame(gtf)
    print(f'reading frames calculated, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)
    ...
    #find CJs
    print('Starting to search for CJs...',flush=True)
    jlist,noj,stat = nmdj.process_events(gtf,threads=args.threads,start_time=start_time_all)
    print(f'CJs found, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)
    ...
    if not args.no_clustering:
        ...
        event_groups = jlist.groupby('gene_id',sort=False,observed=True).apply(nmdj.group_events).reset_index().drop('level_1',1)

        results = event_groups.merge(jlist)

        nmdid = results.groupby(['gene_id','event_num'],
                                sort=False,observed=True).nmd_transcript_id.agg(set).apply(sorted).apply(lambda x: ', '.join(x))
        nmdid.name = 'nmd_id_list'

        results = results.groupby(['gene_id','event_num','chrn','strand',
                                   'transcript_biotype','start','end','jtype'],
                                  sort=False,observed=True).agg({'transcript_id':func,'noj':max,
    ...
    cid = cid.groupby('event_id',sort=False,observed=True).transcript_id.agg(func).apply(sorted).apply(lambda x: ', '.join(x))
    ...
    print(f'CJs {cl}written to {out}, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)
    ...
    if args.ref is not None:
        if ':' in args.ref:
            ...
            print(f'Reference transcript set is specified as <{tag}>=<{val}>',flush=True)
            ref = bigann[(bigann.type=='transcript')&(bigann[tag].fillna('').str.contains(val))].transcript_id.unique()
        else:
            print(f'Reference transcript set is specified as a list of transcript ids in {args.ref}',flush=True)
            with open(args.ref,'r') as file:
                ref = file.read().strip().split('\n')
    else:
        print('Reference transcript set is not specified. Gonna use all coding transcripts as a reference',flush=True)
        ref = None
    ...
    res = []
    for i, (key, df) in enumerate(trids.groupby('event_id',sort=False,observed=True)):
    ...
    if len(noj)>0:
        ...
        cid = cid.groupby('event_id',sort=False,observed=True).tr.agg(func).apply(sorted).apply(lambda x: ', '.join(x))
        cid.name = 'coding_id_list'
        ...
        res = []
        for i, (key, df) in enumerate(trids2.groupby('event_id',sort=False,observed=True)):
            v = nmdj.classify_event(df)
            res.append([key,v])
    ...
    print(f'Transcript structures in event regions written to {out}',flush=True)

    out = f'{args.prefix}classification.tsv'
    events.to_csv(out,sep='\t',index=False)
    print(f'Event classification written to {out}, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)

    #results.to_csv(f'{args.prefix}before_coef.tsv',sep='\t',index=False)


    tag_list=[]
    #coef = results.groupby('event_id').apply(nmdj.assign_coefficients,tag_list).reset_index(drop=True)
    coef,coeftag = psi.get_coefs(results,threads=args.threads,start_time=start_time_all,use_nnls=args.use_nnls)
    coeftag.to_csv(f'{args.prefix}coeftags.tsv',sep='\t',index=False)

    for tag in ['pseudo_blob', 'fatal_ir', 'error', 'nnls_norm>0']:
        p = coeftag[coeftag.coef_tags.apply(lambda x: tag in str(x))][['event_id']]
        p.columns = ['item_id']
        p['item_type'] = 'event'
    ...
    res = []
    for key,tmp in catalog.groupby('event_id',sort=False,observed=True):
        if nmdj.get_sum(tmp)==1:
            res.append([key,'event','PSI calculation error'])

    p = pd.DataFrame(res,columns = ['item_id','item_type','reason'])
    problems.append(p.copy())
    catalog = catalog[~catalog.event_id.isin(p.item_id)]
    ...
    #        sample_name = re.split('/|\.',file)[-3]
        ...
        print(f'Quantification finished, results written to {out}, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)



    out = f'{args.prefix}failed.tsv'
    pd.concat(problems).to_csv(out,sep='\t',index=False)
    print(f'Statistic of failures collected, results written to {out}, time: {round((time.time()-start_time_all)/60,2)} min',flush=True)


    print(f'Finished. Total time: {round((time.time()-start_time_all)/60,2)} min',flush=True)
